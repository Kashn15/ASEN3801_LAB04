...
if P1_5 == 1;
    % % Small disturbance with Simulation
    % x_e2 = 0;
    % y_e2 = 0;
    % z_e2 = 0;
    % phi2 = 0;
    ...
    % 
    % odefun = @(t,x) QuadrotorEOM(t, x, g, m, I, d, km, vu, mu, motor_forces_hover);
    % [t41, X41] = ode45(odefun, [0, t], var41);
    % 
    % U41 = repmat([Zc, Lc, Mc, Nc], length(t41), 1); % size = length(t) x 4
    % 
    % PlotAircraftSim(t41,X41,U41,fig5,col5)

% Load Data
RSdata = load('RSdata_nocontrol.mat');

state_vector = RSdata.rt_estim;
omega_motor = RSdata.rt_motor;
time = RSdata.rt_estim.time(:);

PlotAircraftSim(time, state_vector, omega_motor, fig5, col5)
